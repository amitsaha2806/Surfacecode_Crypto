...
class QuantumArchitecture:
	def __init__(
		...
		'''''
ibm_machine = QuantumArchitecture(
    qubits_per_layer=15,                # Distillation qubits per layer (projected)
    qubits_per_final_distill=16,         # Final distillation qubits
    qubits_distance=500,                 # ~2d² (d=15 for low logical error)
    distill_error_constant=35,           # Distillation error scaling (adjust based on code)
    distill_gates=10.0,                  # Gates per distillation (IBM-specific)
    distance_error_constant=0.03,        # Align with surface code threshold (~1e-3)
    distill_cycles=2.0*3.0*1.25,         # Projected distillation cycle time
    qubit_error=0.001,                   # IBM’s physical single-qubit error (~0.1%)
    threshold_error=0.001,               # Surface code threshold (1e-3)
    state_injection_error=0.02,          # Readout error (~2% for IBM)
    time_mem=0.5,                        # Tradeoff (IBM’s focus on parallelism)
    max_fact=1000,                       # Factory limits (projected)
)
		...
		self.Q_per_final_distill = int(qubits_per_final_distill)
	...
	def distill(self,PA):
		...
		circuit_size=self.Distill_cycles*self.Q_per_final_distill
		...
		while self.Distance_error_constant * math.pow(self.p/self.pt, d[0]) > PA:
			...
			for i in range(1, len(d) - 1):
				if sizes['overlap'][i] == 1:
					qubits_per_magic_state += self.Q_distance_constant * (d[i] ** 2)
	...
	def algorithm_cost(self,algorithm,inputs):
		...
		elif algorithm=="Grover-Zuc":
			results=self.grover_zuc(**inputs)
		elif algorithm=="Grover-Zuc-optimize":
			results=self.grover_zuc_optimize(**inputs)
	...
	def grover_zuc(self,Zuc_length):
		N=int(Zuc_length)
		#from grassl et al.
		if (N==128):
			T=7386928*(2**64)
...
def runTests():
    ...
    print("Grover Zuc optimized 128:")
    ...
    print("Grover Zuc optimized 256:")
    print(results_grover_Zuc_optimize_higher)
